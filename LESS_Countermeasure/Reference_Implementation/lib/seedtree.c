                  ...
                  *seed_storage)             // OUTPUT: sequence of seeds to be released
{
   ...
   /*printf("I am in publish_seeds function\n");
	for (int i=0; i<NUM_NODES_OF_SEED_TREE ; i++) {
      printf("%d ",flags_tree_to_publish[i]);
   }
   printf("\n");*/
                      ...
                      const unsigned char salt[HASH_DIGEST_LENGTH])
{
  ...
  // printf("Hi\n");
    ...
    //  printf("Initial c: %d, flags_tree_to_publish[c]: %d\n", c, flags_tree_to_publish[c]);
      h = 0, h_=0;
      ...
      if(h_ == 0){
         ...
         //printf("%d ", c);
         memcpy(seed_tree + SEED_LENGTH_BYTES*c_,
...
int regenerate_leaves_old(unsigned char
                      seed_tree[NUM_NODES_OF_SEED_TREE*SEED_LENGTH_BYTES],
                      const unsigned char indices_to_publish[T],
                      const unsigned char *stored_seeds,
                      const unsigned char salt[HASH_DIGEST_LENGTH])
